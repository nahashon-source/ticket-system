...
<div class="container mx-auto px-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
        ...
        <!-- Total Tickets -->
        <a href="{{ route('admin.tickets.index') }}" target="_blank"
            class="bg-blue-600 text-white rounded-lg p-6 shadow hover:bg-blue-700 transition">
            <h5 class="text-lg font-medium">Total Tickets</h5>
            <p class="text-3xl mt-2">{{ $totalTickets ?? 0 }}</p>
        </a>

        <!-- Open Tickets -->
        <a href="{{ route('tickets.filter.status', 1) }}" target="_blank"
            class="bg-yellow-500 text-white rounded-lg p-6 shadow hover:bg-yellow-600 transition">
            <h5 class="text-lg font-medium">Open Tickets</h5>
            <p class="text-3xl mt-2">{{ $openTickets ?? 0 }}</p>
        </a>

        <!-- High Priority Tickets -->
        <a href="{{ route('admin.tickets.highPriority') }}" target="_blank"
            class="bg-cyan-600 text-white rounded-lg p-6 shadow hover:bg-cyan-700 transition">
            <h5 class="text-lg font-medium">High Priority</h5>
            <p class="text-3xl mt-2">{{ $highPriorityTickets ?? 0 }}</p>
        </a>

        <!-- Closed Tickets -->
        <a href="{{ route('tickets.filter.status', $closedStatusId) }}" target="_blank"
            class="bg-gray-800 text-white rounded-lg p-6 shadow hover:bg-gray-900 transition">
    ...
    <div class="mb-6">
        <a href="{{ route('admin.users.index') }}" target="_blank"
            class="block bg-gray-700 text-white rounded-lg p-6 shadow hover:bg-gray-800 transition">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($recentTickets ?? [] as $ticket)
                        <tr>
                            <td class="px-6 py-4 text-sm text-gray-800">#{{ $ticket->id }}</td>
                            <td class="px-6 py-4 text-sm text-gray-800">{{ Str::limit($ticket->title, 50) }}</td>
                            <td class="px-6 py-4">
                                <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium 
                                    @switch(optional($ticket->status)->name)
                                        ...
                                        @case('Closed') bg-gray-300 text-s', \App\Http\Controllers\Admin\LabelController::class);
    Route::resource('users', \App\Http\Controllers\Admin\UserController::class)->except(['create', 'store']);
                            ...
                            <td class="px-6 py-4">
                                <a href="{{ route('admin.tickets.show', $ticket) }}" target="_blank"
